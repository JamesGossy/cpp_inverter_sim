...
public:
    ...
    // Output is clamped to [-iq_max, +iq_max] to support regenerative braking.
    SpeedController(Params p): iq_max(p.iq_max), pi(p.kp, p.ki, -p.iq_max, p.iq_max) {}
    ...
    float step(float targetSpeed, float actualSpeed, float dt, float iq_limit = -1.0f) {
        ...
        return pi.step(targetSpeed - actualSpeed, dt, -limit, limit);
    }
